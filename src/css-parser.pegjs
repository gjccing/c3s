...
scope_relative_selector_list
  = first:scope_relative_selector S? next:( ',' S? nextSels:scope_relative_selector )* {
    var result = [first];
    for (var i in next) {
...
scope_relative_selector
  = first_comb:combinator? first_comp:compound_selector more:( combinator compound_selector )* {
    var result = [];
    for (var i in more) {
      var item = more[i];
